...
@interface LoopImageScrollView ()<UIScrollViewDelegate>
{
...
@end
@implementation LoopImageScrollView
...
-(instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _pageCountrol = [[UIPageControl alloc]initWithFrame:CGRectMake((w-100)/2, h-20, 100, 20)];
        _pageCountrol.backgroundColor = [UIColor clearColor];
        _pageCountrol.pageIndicatorTintColor = [UIColor whiteColor];
        _pageCountrol.currentPageIndicatorTintColor = [UIColor blackColor];
...
{
    EGOImageView *aImageView = [[EGOImageView alloc]initWithFrame:CGRectMake(i*w, 0, w, h)];
    aImageView.userInteractionEnabled = YES;
    aImageView.showActivityIndicator = NO;
    aImageView.cacheOrNot = YES;
    aImageView.tag = index;
//    aImageView.imageURL = [NSURL URLWithString:_imageURLArray[index]];
    aImageView.image =[UIImage imageNamed:_imageURLArray[index]];
